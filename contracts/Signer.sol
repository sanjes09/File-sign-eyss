...
contract Signer is Ownable{
  ...
  event userUnverification(address owner, uint256 timestamp);
  event userVerificationRequest(address owner, uint256 timestamp);
  ...
  */
  ...
  /**
    @notice request for address verification
    @param _name name of the user/company
  */
  function requestVerification(string memory _name) public{
    require(!verified[msg.sender].valid, "already a verified user");
    ...
    emit userVerificationRequest(msg.sender, block.timestamp);
  }

  /**
    @notice address verification
    @param _owner address of the user/company to verify
  */
  function verify(address _owner) public onlyOwner{
    require(!verified[_owner].valid, "already a verified user");
    require(verified[_owner].requested, "user have not requested");

    verified[_owner].valid = true;

    emit userVerification(_owner, block.timestamp);
  }
