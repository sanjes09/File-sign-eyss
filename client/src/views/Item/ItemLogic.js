    ...
    if(web3){
      ...
      for (const element of search) {
        if(element.returnValues.id === id){
          ...
          setMetadata(`https://ipfs.io/ipfs/${x.split("ipfs://").join("")}`);
    ...
    try {
      if(web3 && seaport){
        ...
        sellOrders.orders.forEach(element => {
          if((Number(element.currentPrice)/(10**18))/Number(element.quantity) > max) max = (Number(element.currentPrice)/(10**18))/Number(element.quantity);
        });
        setItemPrice(max);
